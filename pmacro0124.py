...
async def bashwait(xname,y):
    cmd = "bash {}".format(pm_go)
    folder=r"{0}/{1}/{2}".format(current,xname,str(y))
    pm = sp.Popen(cmd,shell=True,cwd=folder)
    print("{0}_{1}waiting".format(xname,str(y)))
    pm.wait()
    
def commandwait(ilist,jlist):
    loop = asyncio.get_event_loop()
    task = asyncio.gather(*[bashwait(ilist[x],y) for x in range(2,len(ilist)) for y in range(1,len(jlist[x]))])
...
class Para:
    ...
    def bash_pmdone(self):
        ...
        if __name__ == '__main__':
            commandwait(self.namechangelist,self.datachangelist)
